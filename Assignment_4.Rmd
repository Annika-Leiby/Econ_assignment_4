...
##### C) Going back to the original parameters assume that we could completely prevent climate change (T=0) but doing so would require giving up a fraction (0) of consumption every year for the next 200 years. Find the maximum value of 0 society could endure to prevent climate change. 
...
We plugged in fractional increments of 0.1 from 0 to 1 and calculated the pv for each increment using the facrtion functions created in step 1. We compared these pv values to the pv we calculated when T = 4.4 and o = 0 and found the fractional increment that gave us a pv identical or nearly identical to 198.661. We found that this maximum value of theta (o) to prevent climate change is **0.4**. 
